...
pub struct Enr<K: EnrKey> {
    ...
    /// If true, this record was constructed from an externally-verified source
    /// (e.g. an Archivist SPR with valid libp2p signature) and should not be
    /// re-verified using the ENR signature scheme.
...
impl<K: EnrKey> Enr<K> {
    ...
    /// Construct an ENR from a verified Archivist SPR (SignedPeerRecord).
    ///
    /// The SPR is a libp2p SignedEnvelope (protobuf) containing a PeerRecord.
    /// This constructor verifies the SPR signature, extracts the secp256k1
    /// public key and addresses, then builds an ENR with the correct NodeId.
    ///
    /// The resulting ENR has `externally_verified = true` — its ENR-format
    /// signature is not valid, but the record's identity was verified via
    /// the SPR's libp2p signature scheme.
    pub fn from_verified_spr(
        secp256k1_pubkey: &[u8],
        seq: u64,
        ip4: Option<std::net::Ipv4Addr>,
        udp4: Option<u16>,
        tcp4: Option<u16>,
    ) -> Result<Self, Error> {
        ...
        // id = "v4"
        let mut id_bytes = alloy_rlp::bytes::BytesMut::new();
        b"v4".as_ref().encode(&mut id_bytes);
        content.insert(ID_ENR_KEY.to_vec(), Bytes::from(id_bytes));

        // secp256k1 public key
        let mut pk_bytes = alloy_rlp::bytes::BytesMut::new();
        secp256k1_pubkey.encode(&mut pk_bytes);
        content.insert(b"secp256k1".to_vec(), Bytes::from(pk_bytes));

        if let Some(ip) = ip4 {
            let mut buf = alloy_rlp::bytes::BytesMut::new();
            ip.encode(&mut buf);
            content.insert(IP_ENR_KEY.to_vec(), Bytes::from(buf));
        }
        if let Some(port) = udp4 {
            let mut buf = alloy_rlp::bytes::BytesMut::new();
            port.encode(&mut buf);
            content.insert(UDP_ENR_KEY.to_vec(), Bytes::from(buf));
        }
        if let Some(port) = tcp4 {
            let mut buf = alloy_rlp::bytes::BytesMut::new();
            port.encode(&mut buf);
            content.insert(TCP_ENR_KEY.to_vec(), Bytes::from(buf));
        }
        ...
        Ok(Self {
            ...
            node_id,
            content,
            signature: Vec::new(), // No ENR signature — verified via SPR
